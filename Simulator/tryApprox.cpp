...
void quantizeWeight(v<v<v<v<float>>>>& weightVector, v<v<v<v<int>>>>& quantizedWeightVector){
  ...
  for (int output_channel = 0; output_channel < weightVector.size(); output_channel++){
    for (int input_channel = 0; input_channel < weightVector[0].size(); input_channel++){
      for (int kernel_height = 0; kernel_height < weightVector[0][0].size(); kernel_height++){
        for (int kernel_width = 0; kernel_width < weightVector[0][0][0].size(); kernel_width++){
          quantizedWeightVector[output_channel][input_channel][kernel_height][kernel_width] = maxValue != 0 ? round(127 * weightVector[output_channel][input_channel][kernel_height][kernel_width] / maxValue) : 0;
...
void quantizeInput(v<v<v<float>>>& inputValues, v<v<v<int>>>& quantizedInputValues){
  ...
  for (int input_channel = 0; input_channel < inputValues.size(); input_channel++){
    for (int input_height = 0; input_height < inputValues[0].size(); input_height++){
      for (int input_width = 0; input_width < inputValues[0][0].size(); input_width++){
        quantizedInputValues[input_channel][input_height][input_width] = maxValue != 0 ? round(127 * inputValues[input_channel][input_height][input_width] / maxValue) : 0;
      }
    }
  }
  return;
...
int ExecNeuralNetwork(v<v<v<int>>>& quantizedInputValues, v<v<v<v<v<int>>>>>& quantizedWeightVectors, int approxThreshold){
  ...
  for (int layerIndex = 0; layerIndex < quantizedWeightVectors.size(); layerIndex++)
  {
    ...
    // simulator::computeConv(layerInputValues, quantizedWeightVectors[layerIndex], outputValues, stride);
    simulator::computeConvWithApprox(layerInputValues, quantizedWeightVectors[layerIndex], outputValues, stride, approxThreshold);
    ...
    for (int output_channel = 0; output_channel < num_output_channel; output_channel++){
      for (int output_height = 0; output_height < num_output_height; output_height++){
        for (int output_width = 0; output_width < num_output_width; output_width++){
          layerInputValues[output_channel][output_height][output_width] = outputValues[output_channel][output_height][output_width];
        }
      }
    }
    ...
    for (int input_channel = 0; input_channel < layerInputValues.size(); input_channel++){
      for (int input_height = 0; input_height < layerInputValues[0].size(); input_height++){
        for (int input_width = 0; input_width < layerInputValues[0][0].size(); input_width++){
          layerInputValues[input_channel][input_height][input_width] = maxValue != 0 ? round(127 * layerInputValues[input_channel][input_height][input_width] / (float)maxValue) : 0;
        }
      }
    }
...
int main(int argc, char** argv)
{
  ...
  for (int approxThreshold = 0; approxThreshold < 16; approxThreshold++){
    std::cout << "approxThreshold: " << approxThreshold << std::endl;
    int maxSample = 100;
    int correctSample = 0;
    int sample = 0;
    ...
    for (torch::data::Example<> &batch : *data_loader)
    {
      ...
      for (int64_t i = 0; i < batch.data.size(0); ++i)
      {
        ...
        int classifiedResult = ExecNeuralNetwork(quantizedInputValues, quantizedWeightVectors, approxThreshold);
